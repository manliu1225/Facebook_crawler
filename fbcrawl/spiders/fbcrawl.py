...
class FacebookSpider(scrapy.Spider):
    ...
    def parse_home(self, response):
        ...
        response = scrapy.Request(url=href,callback=self.parse_page,meta={'index':1})
    ...
    def parse_post(self,response):
        self.logger.info("parse_post is {}".format(response.url))
    ...
    def parse_reactions(self,response):
        # print("parse_reactions", response)
        new = ItemLoader(item=FbcrawlItem(),response=response, parent=response.meta['item'])
        new.context['lang'] = self.lang           
